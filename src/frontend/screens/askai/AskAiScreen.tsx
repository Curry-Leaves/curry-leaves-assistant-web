...
export const AskAiScreen = memo(function AskAiScreen({ agents, active: tabActive = true, onOpenNote, focusSessionId, onFocusHandled }: { agents: Agent[]; active?: boolean; onOpenNote?: (path: string) => void; focusSessionId?: string | null; onFocusHandled?: () => void }) {
  ...
  // Scroll the thread's OWN container to the bottom — never `endRef.scrollIntoView()`,
  // which walks up every scrollable ancestor to the document and, on first mount when the
  // sentinel is below the fold, scrolls the whole app up so the fixed top bar is clipped.
  const scrollThreadToEnd = (smooth = true) => {
    const el = threadRef.current;
    if (el) el.scrollTo({ top: el.scrollHeight, behavior: smooth ? 'smooth' : 'auto' });
  };
  useEffect(() => { scrollThreadToEnd(); });
  ...
  const attachRun = (runId: string, sessionId: string, mirror: boolean) => {
    ...
    stop = subChat(runId, (raw) => {
      ...
      if (openSessionId.current === sessionId) scrollThreadToEnd();
  ...
  return (
    <div className="flex-1 flex min-h-0 min-w-0">
      ...
      {/* thread */}
      <div ref={threadRef} className="relative flex-1 overflow-y-auto px-6 py-6 min-h-0">
